...
# ==========================================
    ...
    if st.button("🔮 Predict Remaining Useful Life", type="primary"):
        try:
            # 1. تحميل الموديل والـ Transformers
            ...
            # 2. الخطوة الذكية: استخراج أسماء الحساسات اللي الموديل ده متدرب عليها بالظبط
            expected_features = list(pt.feature_names_in_)
            
            # 3. تجميع كل مدخلات اليوزر (الـ 24 عمود)
            input_df = pd.DataFrame([user_inputs])
            
            # 4. الفلترة التلقائية: بناخد بس العواميد اللي الموديل محتاجها ونتجاهل الباقي
            input_filtered = input_df[expected_features]
            
            # 5. تطبيق الـ Scaling والتوقع
            input_transformed = pt.transform(input_filtered)
            input_scaled = scaler.transform(input_transformed)
            
            # استخدمنا input_scaled كأرقام عشان LightGBM ميعملش أي Warning
