...
# ==========================================
    ...
    # هنرجع الـ 21 حساس زي ما هما عشان الواجهة تبقى كاملة وطبيعية للـ User
    ...
    if st.button("🔮 Predict Remaining Useful Life", type="primary"):
        try:
            ...
            selected_features = artifacts['selected_features'] # (دول الـ 4 عواميد)
            
            # 2. تجميع كل مدخلات اليوزر من الواجهة
            input_df = pd.DataFrame([user_inputs])
            
            # 3. معرفة العواميد اللي الـ PowerTransformer اتدرب عليها (الـ 23 عمود)
            pt_expected_features = list(pt.feature_names_in_)
            
            # 4. نفلتر مدخلات اليوزر عشان تناسب الـ PowerTransformer ونعمل Transform
            input_for_pt = input_df[pt_expected_features]
            pt_transformed = pt.transform(input_for_pt)
            
            # 5. نرجع النتيجة لـ DataFrame عشان نعرف نسحب منه العواميد المهمة بس
            pt_transformed_df = pd.DataFrame(pt_transformed, columns=pt_expected_features)
            
            # 6. نسحب العواميد المهمة بس (الـ 4 عواميد) اللي الموديل والـ Scaler محتاجينها
            final_input = pt_transformed_df[selected_features]
            
            # 7. تطبيق الـ Scaler الأخير
